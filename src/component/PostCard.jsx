...
function PostMenu({ onAffiliateClick }) {
    ...
    <div className="relative">
      ...
      {menuOpen && (
        <>
          ...
          <div className="absolute right-0 top-8 z-50 bg-white rounded-xl shadow-xl border border-gray-200 w-48 py-1">
            <button
              onClick={(e) => {
                e.stopPropagation()
                setMenuOpen(false)
...
function PostCard({ post, showStar = false, onAffiliateSelect }) {
  ...
  const [affiliateImageIndex, setAffiliateImageIndex] = useState(0)
  ...
  const handleAffiliateSelect = (product) => {
    if (onAffiliateSelect) {
      onAffiliateSelect(post.id, product, affiliateImageIndex)
    }
...
{/* Three dots menu - only on profile posts */}
               {showStar && (
                 <PostMenu
                   onAffiliateClick={() => {
                     setAffiliateModalOpen(true)
                     setAffiliateImageIndex(currentImage)
                   }}
                 />
               )}
            </div>
            ...
            {/* Product Card - tied to current image */}
...
{/* Affiliate Modal */}
      <AffiliateModal
        ...
        onSelect={handleAffiliateSelect}
      />
      ...
      <ImageModal
        ...
        onPrev={(e) => { e.stopPropagation(); prevImage(e); }}
        onNext={(e) => { e.stopPropagation(); nextImage(e); }}
        showStar={showStar}
        onStarClick={() => {
